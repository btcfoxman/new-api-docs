...
<CardGroup cols={2}>
  <Card title="統一接入" icon="code">
    優先提供 OpenAI 相容格式，已有基於 OpenAI SDK 或 HTTP 的應用通常只需要替換 Base URL 和 API Key。
  </Card>
  <Card title="多種視訊協定" icon="film">
    同時公開 `/v1/videos`、Seedance 原生格式、Omni 與 Veo 格式說明，以及部分模型專用影片介面。
  </Card>
  <Card title="圖像能力" icon="image">
    提供 OpenAI 圖片生成與編輯介面，並保留 Gemini 圖片生成等模型專用格式。
  </Card>
  <Card title="音樂生成" icon="music">
    提供音樂生成任務介面，支援提示詞描述、輕音樂/純伴奏、歌詞高級模式，並可透過任務 ID 查詢結果。
  </Card>
  <Card title="長任務查詢" icon="clock">
    影片、音樂與部分生成任務統一使用任務 ID、response id 或 operation name 查詢，適合非同步整合與回查。
  </Card>
</CardGroup>
