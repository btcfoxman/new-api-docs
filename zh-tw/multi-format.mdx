---
title: "多協定與多格式接入說明"
description: "當前專案並非僅支援單一協定。為降低整合成本，公開文件優先提供統一格式；同時保留少量模型原生格式或專用格式。"
---
...
- HappyHorse：`/api/v1/services/aigc/video-generation/video-synthesis` 和 `/api/v1/tasks/{task_id}`
- Vidu：`/ent/v2/text2video`、`/ent/v2/img2video`、`/ent/v2/start-end2video`、`/ent/v2/reference2video` 和 `/ent/v2/tasks/{task_id}/creations`
- Omni 與 Veo：`gemini-omni` 使用 `/v1/videos` 或 `/api/v3/contents/generations/tasks`；Veo 原生預留路徑為 `/v1beta/models/{model}:predictLongRunning` 和 `/v1beta/{operation_name}`
- Gemini 圖片生成：`/v1beta/models/{model}:generateContent`
- 音樂生成：`/suno/submit/music` 和 `/suno/fetch/{task_id}`，支援提示詞描述模式、輕音樂/純伴奏、歌詞進階模式
