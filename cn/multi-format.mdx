...
- Omni 与 Veo：`gemini-omni` 使用 `/v1/videos` 或 `/api/v3/contents/generations/tasks`；Veo 原生预留路径为 `/v1beta/models/{model}:predictLongRunning` 和 `/v1beta/{operation_name}`
- Gemini 图片生成：`/v1beta/models/{model}:generateContent`
- 音乐生成：`/suno/submit/music` 和 `/suno/fetch/{task_id}`，支持提示词描述模式、轻音乐/纯伴奏、歌词高级模式
